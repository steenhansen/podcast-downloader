...
begin
  ...
  begin
   ...
   number_episodes:= fileSelectionMediator.readRssFile(edtSaveDirectory, podcastForm, lblPodcastDescription, @DoOnReadPodcast, @DoOnFailedReadPodcast);
  ...
  if totalFileSize>0 then
     ...
     Result := number_episodes;         // Since sffAudio PDFs have no file size we must do this cheat.
...
begin
  ...
  begin
    ...
    fileSizeChecked := fileSelectionMediator.calcDownload(clbEpisodeFiles,
      btnDownloadChecked);
  end;
...
begin
  ...
  begin
    ...
    totalFileSize := fileSelectionMediator.clickedBoxNew(clbEpisodeFiles,
      lbEpisodeDesc, Index, btnDownloadChecked);
  end;
  Result := totalFileSize;
end;
...
begin
  ...
  with PodcastForm do
  begin
    failsAndSuccesses := fileSelectionMediator.startDownloading(
      lbEpisodeDesc, @DoOnWriteEpisode, @DoOnFailedReadEpisode, podcast_file, saveDir);
  end;
